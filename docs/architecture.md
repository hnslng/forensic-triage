...
Profile liegen als YAML-Dateien im Elternordner des konfigurierten `FORENSIC_TRIAGE_PROFILE`; standardmäßig unter `profiles/` im Codeverzeichnis. Der Profil-Editor schreibt dorthin. Eine automatisch migrierte, releaseunabhängige Profilablage fehlt bisher; dies ist für Updates noch abzusichern. Mehrere ausgewählte Profile werden vor dem Scan zusammengeführt und Begriffe dedupliziert. Die tatsächlich ausgewählte Liste sowie Profilversion und SHA-256-Profilhash werden mit dem Scan gespeichert.

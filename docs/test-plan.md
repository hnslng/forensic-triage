...
7. Run the CLI scanner in default fast mode and verify the temporary mount is read-only and absent after completion.
8. Run `--mode tsk` when validating the mount-free fallback.
9. Compare file count, directory count, extensions, category counts/bytes, keyword counts, and top-five files with `tests/fixtures/expected.json`.
10. Record runtime and retain raw command output.

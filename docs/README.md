...
Stand: 10. September 2026, Anwendung Alpha 49. Zuerst [Projektstand und Nachweise](project-status.md) lesen; danach [Roadmap](roadmap.md) und [vollständigen Probeeinsatz](test-plan.md#vollständiger-probeeinsatz). Implementierte Funktionen, praktische Beobachtungen und offene Abnahmen werden getrennt ausgewiesen.
...
Die Paketversion steht in `pyproject.toml` und `src/forensic_triage/__init__.py`. Die aktuelle Programmversion ist 0.2.0-alpha.49. Dafür lautet die Python-Version gemäß PEP 440 `0.2.0a49`; der Git-Tag lautet `v0.2.0-alpha.49`.

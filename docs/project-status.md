...
Stand: **10. September 2026**, Anwendung **v0.2.0-alpha.48**. Einstellungen für Stichwortprofile und Dateitypen, signierte Offline-Updates sowie kontrollierte Systemaktionen und Pi-Stromstatus sind implementiert. Der erste echte `.tbu`-Versuch Alpha 45 → 46 erreichte Signaturprüfung und Staging, scheiterte aber bei der Installation ohne `setuptools`; Alpha 48 enthält den dafür nötigen Offline-Bootstrap. Einzelheiten: [Einstellungen](settings.md) und [Offline-Updates](offline-updates.md).
...
|---|---|---|
| Raspberry Pi 3B+ | Installiert und im Testbetrieb; Alpha 45 wurde am 10. September online installiert, einschließlich 123 erfolgreicher Python-Tests auf dem Pi. | Alpha 48 per `.tbu` aktualisieren; reproduzierbare Neuinstallation, finaler Speicher-/Stromaufbau und Dauerbetrieb. |
...
| Updates | Online-Prüfung und bewusste Installation praktisch verwendet; Alpha 45 wurde von Alpha 43 aus erfolgreich installiert. Der erste Offline-Versuch Alpha 45 → 46 legte ein fehlendes Build-Modul offen; Alpha 47 installierte, scheiterte aber an einem vom systemd-Arbeitsordner abhängigen Selbsttest. Der korrigierte Alpha-48-Pfad wird lokal aus `/` reproduziert. | Alpha 45 → 48 auf dem Pi praktisch abschließen, danach Fehler-/Stromausfallprüfung und Wiederherstellung aller veränderten Komponenten. |
...
- **133 Python-Tests:** für Alpha 48 erfolgreich; zusätzlich sind Stromstatusbits, fehlende Pi-Werkzeuge, feste Systemaktionen und serverseitige Arbeitssperren abgedeckt.
- **28 isolierte Browsertests:** für Alpha 48 erfolgreich. Sie prüfen zusätzlich Stromanzeige, zweistufige Bestätigung und schmale Darstellung.
...
Für Alpha 48 wurden noch keine neuen realen Scans, Offline-Updates am Pi, Stromunterbrechungen oder Wiederherstellungen ausgeführt. Neue Messwerte gehören mit Version, Aufbau, Testbestand und Abweichungen in einen datierten Nachweis; echte Falldaten bleiben außerhalb von Git.
